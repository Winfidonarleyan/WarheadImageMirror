...
{
    // Default function
    [[noreturn]] WH_COMMON_API void Assert(std::string_view file, int line, std::string_view function, std::string_view message, std::string const& fmtMessage = "");
...
#define WPAssert(cond, ...) do { if (!(cond)) Warhead::Assert(__FILE__, __LINE__, __FUNCTION__, #cond, ##__VA_ARGS__); } while(0)
